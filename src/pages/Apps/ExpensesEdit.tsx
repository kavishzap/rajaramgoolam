...
const ExpensesEdit = () => {
        ...
        const fetchExpense = async () => {
            ...
            try {
                const { data, error } = await supabase
                    .from('expenses')
                    .select('expense_date, expense_note, expense_items, expense_total')
                ...
                setNote(data.expense_note || '');

                const itemsArr: ExpenseLineItem[] = Object.entries(data.expense_items || {}).map(
                    ([name, value], index) => {
                        const amountNum = parseFloat(String(value)) || 0;
                        // We don't have separate qty/rate in DB; assume qty=1, rate=amount
                        return {
                            id: index + 1,
                            title: name,
                            description: '',
                ...
                setItems(itemsArr.length ? itemsArr : [
    ...
    const updateItemField = (lineId: number, field: keyof ExpenseLineItem, value: string) => {
        setItems((prev) =>
            prev.map((item) => {
    ...
    const handleSave = async () => {
        if (!userEmail || !id) {
            Swal.fire('Error', 'No authenticated user or expense id. Please try again.', 'error');
            return;
        }
        ...
        try {
            ...
            const { error } = await supabase.from('expenses').update(payload).eq('id', Number(id)).eq('expense_company_email', userEmail);
            Swal.close();
